---
name: daybreak-advisor
description: Run a software-delivery request through Sol Advisor with GPT Daybreak Blue / High as the primary orchestrator.
---

# Daybreak Advisor shorthand

Treat the text after `$daybreak-advisor:` as the user's task request.
...
- Every requirement for the **primary session** to use GPT-5.6 Sol / High becomes a
  requirement to use GPT Daybreak Blue (`gpt-daybreak-blue-latest`) / High.
...
fresh `sol_advisor_sol_reviewer` role at Sol / High; do not substitute a Daybreak
reviewer.

Verify the primary model and effort when runtime metadata exposes them. If either
differs, tell the user to select Daybreak Blue / High and stop before task work or
delegation. If runtime metadata does not expose them, ask the user to confirm
Daybreak Blue / High and stop until confirmed. A skill cannot change the primary
model itself; never assume or claim this prerequisite is satisfied.
