...
5. **Quota Protection & Graceful Failover**:
   Everyday tasks run on Gemini Flash 3.8 and Pro 3.1 to conserve Claude quota. Claude can only hold the **chair** here — every invocable subagent runs a Gemini model — so quota exhaustion costs the run its Claude chair, and with it the cross-vendor tier. Log the failover naming both lanes and the independence actually carried. An **implementer** may fail over immediately — it carries no independence claim. A **reviewer** may not: every invocable subagent lane here runs a Gemini model, so every reviewer failover lowers the tier, and lowering it is the user's call. Stop and ask. Never fail over to the model the chair is running: that makes reviewer and orchestrator the same model, which is the absence of independent review rather than a weaker form of it.
